...
function Dino(dino_attrs) {
  this.name = dino_attrs.name,
  this.diet = dino_attrs.diet,
  this.weight = dino_attrs.weight,
  this.length = dino_attrs.length,
  this.period = dino_attrs.period
}
...
// tyrannosaurus, carnivorous, 7000kg, 12m, Late Cretaceious
  ...
  console.log('tyrannosaurus', tyrannosaurus)
...
stegosaurus = new Dino({
  ...
  console.log('stegosaurus', stegosaurus)

// velociraptor, carnivorous, 15kg, 1.8m, Late Cretaceious
  velociraptor = new Dino({
    name:'velociraptor',
    diet:'carnivorous',
    weight:'15kg',
    length:'1.8m',
  ...
  console.log('velociraptor', velociraptor)
...
// -- get school names -- //
  const universities = [];
  const subList_universityNames = graduates.forEach(graduate => {
...
*/
  ...
  zooAnimals.forEach(animal => {
    animalNames.push(`Animal Name: ${animal.animal_name} / Scientific Name: ${animal.scientific_name}`)
  })
  console.log(animalNames);

/* Request 2: .map()    
  The zoos need a list of all their animal's names (names only, not scientific) converted to lower case.  Create a new 
  array named lowerCase and map over each name to convert them all to lower case.  Log the result.
*/
...
/* Request 3: .filter() 
  The zoos are concenred about animals with a lower population count. Find out which animals 
